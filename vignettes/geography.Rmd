...
2.  If sources have incomplete information on the location, researchers are to triangulate information through following landmarks or
   relevant information mentioned by the source, and utilize the precision code (see below) to denote that.

3.  If sources include administrative levels which are not part of the standardized list of administrative levels of ACLED, then researchers are allowed to include new entries to out master list by checking OpenStreetMap, Google Maps and NGA GeoNames database.
...
Very often sources have different degrees of certainty regarding an event occurring in a particular location. For instance, some sources may report an event as "near the 66 highway near City X", while others may report it at "the event occurred at the town hall of city X along the 66 highway". In that example, we can see two different degrees of
precision. To distinguish these different degrees of precision, as with the case of [time](https://billingtt.github.io/acledR/articles/time.html), ACLED utilizes different "Precision Codes". These are as follow:
...
We start by pulling the data out of ACLED's API. First, we call `acled_acess()`, and then we make our API call with `acled_api()`. Lets explore which are the top 5 admin1s of Nigeria (also called states).
...
As observed by our table, Kaduna and Borno are the two most conflict-ridden admin1s in Nigeria, however, conflicts appears generally evenly distributed from there on. Lets plot the geographical distribution:
