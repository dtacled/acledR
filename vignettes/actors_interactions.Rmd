...
vignette: >
  %\VignetteIndexEntry{Actors and Interactions}
...
During this month, there were `length(df_yemen$data_id)` ACLED events, `length(unique(df_yemen$actor1))` unique `actor1` entries, and `length(unique(df_yemen$actor2))` unique `actor2` entries. 
