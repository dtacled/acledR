...
MENTION THAT EVERY TIME THAT A FUNCTION WORKS WITH WEEK IT WORKS WITH ACLED WEEKS, SO TAHT THEY KNOW. (WE START ON SATURDAY AND NOT MONDAY)
...
If sources include an actual date of an event, a time precision code of "1" is recorded. If sources note that an event happened during a specific week or in the weekend, "2" is noted in the time precision column and the middle of that week (or of the weekend) is used as the reference date. If sources note only that an event took place within a particular month, without reference to the particular date, the month mid-point is chosen unless the beginning or end of the month is noted (in which case, the first and last date are used, respectively) and "3" is noted as the time precision level. ACLED does not include events with less temporal information.
...
In order to explore the concept of time in ACLED's data, we can look at the following example, where we are analyzing France's events throughout 2021. 
