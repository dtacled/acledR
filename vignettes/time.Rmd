...
french_counts <- generate_counts(df_france,unit_id="country",time_id="event_date",time_target = "month")%>%
  select(-country) %>%
  pivot_longer(!event_month,names_to="event_type",values_to = "n_events") 

pfc <- ggplot(french_counts, aes(x=event_month, y=n_events, fill=event_type, text=event_type))+
...
As we can see from the graph, France's 2021 was primarely riddled with protests. However, this is hardly enough to conclude anything about France. In another vignitte we go through this example and compare it with the regional averages. 
