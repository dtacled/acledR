...
The function thus requires only two arguments `events` and `method`. `events` is a vector of event/fatalities counts per actor (see `acled_generate_counts()` for an easy way to generate such counts.). `method` is the desired methodological approach to calculate concentration indices, either Inverse Simpson Index or Hertindahl-Hirscham Index. These are described below:
