...
The email and key must be those associated with your ACLED account registered at [ACLED's Access Portal](developer.acleddata.com). Once you acquired your credentials (email and key), you can either utilize `acled_access()` to store these in the environment for a given session, or manually store them in the enviorment file to utilize them permanently across every session. 
...
To see how this works in practice, here's an example calling just Brazil since January 2022. Note that I call my email and key directly with `Sys.getenv()` rather than pasting, and I set `acled_acess=FALSE`. If before hand I would have run `acled_access()` then I would have ignored email and key, and set `acled_access = TRUE`.
