...
In both the dyadic and monadic versions, the API returns `r unique(df_sa$event_id_cnty) %>% length()` unique events. However, the number of unique entries for the dyadic version is also `r df_sa$event_id_cnty %>% length()`, while it's `r df_sa_monadic$event_id_cnty %>% length()` for the monadic version.
