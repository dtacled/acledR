...
library(secret)
library(here)
my_vault <- here("vault","secret-vault.vault")
tb_priv_key <- here("vault","secret-vault.vault", "acledR_vault")
EMAIL_ADDRESS = get_secret("EMAIL_ADDRESS", key = tb_priv_key, vault = my_vault)[[1]]
ACLED_KEY = get_secret("ACLED_KEY", key = tb_priv_key, vault = my_vault)[[1]]
```
...
```{r}
library(acledR)
df_br <- acled_api(email = EMAIL_ADDRESS,
                   key = ACLED_KEY,
                   countries = c("Brazil"),
...
```{r}
df_br_co <- acled_api(email = EMAIL_ADDRESS,
                      key = ACLED_KEY,
...
```{r}
df_sa_monadic <- acled_api(email = EMAIL_ADDRESS,
                           key = ACLED_KEY,
                           regions = c("South America"),
                           start_date = "2022-01-01",
                           end_date = "2022-04-30",
