...
With these event counts, we can use `generate_movers()` to calculate moving statistics of these variables. Let's create two new variables, one for the moving average of `total_events` over the last 3 months:
...
We now have two new columns for the two requested moving averages: `total_events_moving_mean_3`. We can look at the first few rows of our variables of interest:
