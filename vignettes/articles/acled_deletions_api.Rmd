...
For users with a personal ACLED file, which appends/adds events regularly (e.g weekly or monthly), it is crucial to check which events are no longer in the actual ACLED dataset. Otherwise, users risk having an outdated version. To satisfy this need, users can use the `acled_deletion_api()` function to gather the *event_id_cnty* of deleted events, which you can use to filter out these events. While users can find an example below, we kindly suggest you to visit our [guide](https://acleddata.com/download/35179/).
...
2\. We don't know if our datasets contains deleted data or not.
...
We can see then that our dataset consists of 326 rows/events. 
...
latest_timestamp <- as_date(latest_timestamp_unix) 

print(latest_timestamp)

```
Now that we know the latest timestamp, we can use that as a reference for gathering deleted events. 

```{r, eval = T, echo=F}
acled_deleted_events <- acled_deletions_api(date_deleted = latest_timestamp, acled_access = T)
```
...
Quite a lot of deleted events, `r nrow(acled_deleted_events)`. Lets check if any of these are part of my dataset.
...
dummy_acled_file <- acledR::acled_deletion_old_dummy_n3 # Here is our personal ACLED dataset

nrow(dummy_acled_file)

```

With this we have loaded the dataset, and we can see that there are `r nrow(dummy_acled_file)` events in it. Now we proceed to retrieve the deleted events since the last time we gather data.
...
As we can see `r nrow(deleted_events)` events have been removed since the last time I uploaded my data. Now that we know there are deleted events, lets check if any of these are in my dataset.
...
As you can see there were deleted events, and we have excluded them through the dplyr::filter() function. Now our dataset has only `r nrow(updated_dataset)` rows, thus `r nrow(dummy_acled_file) - nrow(updated_dataset)` events were deleted.
