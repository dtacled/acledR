...
```{fun , eval = FALSE}
acled_transform_interactions(df, only_inters = F) 
```
...
df_sa <- acled_api(regions = "South America",
                   ...
                   prompt = F)
```
Now that your data are in long format with one actor per row, you can much more easily filter the data to retain only those events involving the "ELN: National Liberation Army": 
