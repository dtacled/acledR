...
test_that("number of columns is correct - date", {
  skip_on_cran()
  expect_equal(ncol(received_deleted_data_date_new),2)
})
...
test_that("number of columns is correct - unix", {
  skip_on_cran()
  expect_equal(ncol(received_deleted_data_unix_new),2)
})
...
test_that("names of columns are correct - date", {
  skip_on_cran()
  expect_equal(names(received_deleted_data_date_new),columns_deleted)
})
...
test_that("names of columns are correct - unix", {
  skip_on_cran()
  expect_equal(names(received_deleted_data_unix_new),columns_deleted)
})
...
test_that("Email and key are handled as expected without acled_access",{
  skip_on_cran()
  expect_true(grepl("t.billing@acleddata.com", received_deleted_log_new$email[1]))}
)
...
test_that("names of columns are correct - unix", {
  ...
  expect_true(all.equal(received_deleted_data_date_new, received_deleted_data_unix_new))
})
