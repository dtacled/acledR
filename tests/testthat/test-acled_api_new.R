...
test_that("number of columns is correct", {
  skip_on_cran()
  expect_equal(ncol(received_data_new),31)
})
...
test_that("names of columns are correct", {
  skip_on_cran()
  expect_equal(names(received_data_new),columns)
})
...
test_that("Split calls for big calls", {
  skip_on_cran()
  expect_equal(as.numeric(ceiling(sum(log_received_data_new$time)/300000)),max(log_received_data_new$calls))
})
...
test_that("Regions in numeric work",
          {
            skip_on_cran()
            expect_true(all.equal(data.frame(region="Western Africa", rows=1:nrow(received_data_numeric_region_new))$region,
                                  received_data_numeric_region_new$region))
          }
)
...
test_that("Testing that when requestion a region, and a country of another region, you get both",{
  ...
  expect_setequal(unique(received_data_country_and_region_new$country), list_countries)
...
test_that("When requesting a region with a numeric input, and a country of another region, you get both",{
  ...
  expect_setequal(unique(received_data_country_and_region_num_new$country), list_countries)
...
#   skip_on_cran()
#   expect_gte(min(timestamp_numeric_check$timestamp), 1673295342)
# })
...
test_that("The call actually returns monadics.", {
  skip_on_cran()
  expect_equal(min(received_data_monadic_new$event_date), min(received_data_new$event_date))

  expect_equal(max(received_data_monadic_new$event_date), max(received_data_new$event_date))

  expect_equal(unique(received_data_monadic_new$country), unique(received_data_new$country))

  expect_gte(nrow(received_data_monadic_new), nrow(received_data_new))
})
...
test_that("Population columns are being received", {
  ...
  received_data_pops_new <- acled_api(email = Sys.getenv("ACLED_API_EMAIL"),
                                      password = Sys.getenv("ACLED_API_PASSWORD"),
                                  country="Argentina", start_date="2022-01-01",end_date = "2022-01-04",
                                  population='full',
  ...
  received_data_best_new <- acled_api(email = Sys.getenv("ACLED_API_EMAIL"),
                                      password = Sys.getenv("ACLED_API_PASSWORD"),
                                  country="Argentina", start_date="2022-01-01",end_date = "2022-01-04",
                                  population='best',
                                  # prompt = F, acled_access = F,
  ...
  expect_true(all(population_cols %in% colnames(received_data_pops_new)))
  expect_true("population_best" %in% colnames(received_data_best_new))
...
test_that("Tables display NAs instead of blanks", {
  ...
  expect_equal(nrow(filter(received_data_new, if_any(everything(), ~ sjmisc::is_empty(.x, all.na.empty = F)))), 0)
