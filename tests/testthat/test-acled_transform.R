...
test_that("returns a tibble", {
  ...
  output <- acled_transform(data, "full_actors")
...
test_that("returns correct number of rows for main_actors type", {
  ...
  output <- acled_transform(data, "main_actors")
...
test_that("returns correct number of rows for assoc_actors type", {
  data <- data.frame(actor1 = c("Actor D"), actor2 = c("Actor C"), assoc_actor_1 = c("Actor A; Actor B"), assoc_actor_2 = c("hello"), inter1 = "1", inter2="2",sub_event_type = "Protests", source_scale= "khapow",source= 'khapow')
  output <- acled_transform(data, "assoc_actors")
...
test_that("returns correct number of rows for source type", {
  ...
  output <- acled_transform(data, "source")
  expect_equal(nrow(output), 2)
})

# Test that the function returns the correct number of rows for all type
test_that("returns correct number of rows for all type", {
  data <- data.frame(actor1 = c("Actor D"), actor2 = c("Actor C"), assoc_actor_1 = c("Actor A; Actor B"), assoc_actor_2 = c("Hello"),inter1 = "1", inter2="2", sub_event_type = "Protests", source_scale= "khapow", source = c("Source A; Source B"))
  output <- acled_transform(data, "all")
...
test_that("throws an error for invalid type parameter", {
  ...
  expect_error(acled_transform(data, "invalid_type"))
})
...
test_that("if columns are missing, it will throw an error for an invalid data frame", {
  ...
  expect_error(acled_transform(data, "full_actors"), "Some columns are missing. Please make sure your data frame includes: actor1, actor2, assoc_actor_1, assoc_actor_2, sub_event_type, source_scale, source.")
})
...
test_that("error if the actor1 or actor 2 columns have more than one value per row", {
  ...
  expect_error(acled_transform(data, "main_actors", "*column seems to include more than one result per row. That is inconsistent with our column structure."))
})
...
test_that("warning when there are empty rows in the assoc actors column", {
  ...
  expect_warning(acled_transform(data, "assoc_actors"), "There are empty rows in the assoc_actor column.")
  expect_warning(acled_transform(data, "full_actors"), "There are empty rows in the actor column.")
  expect_warning(acled_transform(data, "all"), "There are empty rows in the actor column.")
})
